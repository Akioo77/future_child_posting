...
def slide_project_rq(prs):
    ...
    features = [
        ...
        ("✍️ 改写文案", "AI 生成脱敏后的替代文案（后续扩展功能，当前版本仅提供修改建议）"),
    ...
    tech = [
        ("文字扫描", "LLM（qwen3.6-plus 多模态模型）+ 规则匹配"),
        ("图片扫描", "VLM（qwen3.6-plus 多模态模型，支持图片+文字联合分析）"),
...
def slide_mvp_upload(prs):
    ...
    features = [
        ...
        ("🚀 一键分析", "点击「开始分析」按钮，触发 AI 风险扫描，显示加载状态"),
        ("⏳ 等待过程", "分析过程中显示「分析中...」状态提示（后续可扩展进度条）"),
    ]
    y = Inches(1.65)
    ...
    details = [
        ...
        ("渐进式引导", "使用说明区介绍基本操作，降低首次使用门槛（后续可扩展新手引导）"),
...
def slide_mvp_report(prs):
    ...
    risks = [
        ...
        ("R4", "位置暴露风险", "可识别地理位置的建筑、地标、环境信息（如学校标志、标志性建筑、门牌号）", C_PURPLE),
